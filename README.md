...
Trong chat Claude Code, gõ `/` để xem danh sách commands có sẵn.
...
| Command | Khi nào dùng |
|---------|-------------|
| `/analyze_requirement_document` | Phân tích Jira ticket / doc — phát hiện ambiguities, sinh checklist AC. **Không sinh TC** |
...
| `/generate_bug_report` | Chuẩn hóa bug report sẵn sàng paste vào Jira |
...
4. Output: Bảng test cases Markdown sẵn sàng copy sang Excel / Jira / TestRail
...
Mô tả lỗi bằng ngôn ngữ tự nhiên → nhận bug report chuẩn sẵn sàng paste vào Jira.
...
Cung cấp Jira ticket hoặc file spec → nhận danh sách ambiguities + AC checklist.
...
- Với FULL RBT: **không bỏ qua bước nào**, đặc biệt Bước 2 (Q&A với PO) là điểm quan trọng nhất
